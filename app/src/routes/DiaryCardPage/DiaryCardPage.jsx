	...
	return (
		<div className="diaryCardPage_blur_wrapper" onLoad={scrollPos}>	
			...
			<div className="diaryCardPage_container" onLoad={setRatingEvents}>
				...
				<div className="diaryCardPage_teaPicture_block"><div className="diaryCardPage_teaPicture_shape"><img draggable="false" src={teaObj.teaImgPath} alt="tea img" /></div></div>
                <p className='diaryCardPage_userReviewDate_info '>{teaObj.userReviewDate} в {teaObj.userReviewTime}</p>
                ...
                <div className="tea_info">
						<p className="tea_notation captionOne-font">{teaObj.userReviewText}</p>
				</div>
                <div className='diaryCardPage_userPhotos_block'>
                    {teaObj.reviewPictures.map(item => 
                        // <img key={nanoid()} src={item} />
                        <Fancybox key={nanoid()} options={{ infinite: false }}><img data-src={item} data-fancybox="gallery" draggable="false" className="user-review-img" src={item} alt="img" /></Fancybox>    
                    )}
                </div>
                <div className="diaryCardPage_button_container">
                    ...
                    <BtnSecond teaid={teaObj.teaId} onClick={props.deleteCard}>Удалить</BtnSecond>
                    <BtnDefault>Посмотреть все о чае</BtnDefault>
                </div>
			</div>
		</div>
	)
