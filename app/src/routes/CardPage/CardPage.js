...
export default function CardPage(props) {
		...
		return (
			<form onSubmit={(e) => e.preventDefault()}>
				...
				<div className="review-option">
					...
					<TokenSecond><label className="review-filter-button button-wanted buttonOne-font" htmlFor="adding_review_filter_checkbox_id_cardPage_2" onClick={() => props.addFavouriteTea(props.teaObj)}><span className='cardPage_heart_span'>♡</span> Избранное</label></TokenSecond>
				</div>
			</form>
		)
	}
	...
	function checkIsAdded() {
		if(props.user !== null) {
			if(props.user.tea_favourives.map(item => item.id).includes(props.teaObj.id)) {
				setAddedBox(<WantedTeaForm />)
			}
			else if(props.user.tea_collection.map(item => item.parentTea.id).includes(props.teaObj.id)) {
	...
	function collectAddingData(data) {
			setTastedAdding(null)
			setAddedBox(<TastedTeaForm />)
			console.log('Data collected', data);
			props.addCollectionTea(props.teaObj, data)
	}
