...
export default function ProfilePage(props) {
    // props.auth.user == null ? props.user = :  
    if(props?.user) {
        var currentUser = props.user;
        if(currentUser.firstName === '' || !currentUser.firstName) {
            currentUser.firstName = "Чайный гость"
        }
    }
    const [categoryTeasCount, setCategoryTeasCount] = useState(`${currentUser.tea_collection.length} коллекция`)
    const [userTeas, setUserTeas] = useState(null)
    const [tastedTeaContainer, setTastedTeaContainer] = useState(<DiaryCardContainer containerId="tastedTeaContainer" currentTeasContainerCategory='Коллекция' teaObj={currentUser.tea_collection} />)
    const [wantedTeaContainer, setWantedTeaContainer] = useState(<DiaryFavCardContainer containerId="wantedTeaContainer" user={currentUser} currentTeasContainerCategory='Желания' teaObj={currentUser.tea_favourives} />)
    ...
    function changeCategory() {
        if(wanted_option_state === true) {
            ...
            setCategoryTeasCount(`${currentUser.tea_collection.length} в коллекции`)
            ...
            setCategoryTeasCount(`${currentUser.tea_favourives.length} желания`)
            ...
            <div className="profilePage_profileMainWrapper">
                ...
                <div className="profilePage_userInfo">
                    <div className="profilePage_userAvatar_block">
                        {currentUser?.userpic ? <img className='profilePage_userImg' src={currentUser.userpic} alt="Avatar" /> : <SmileAvatar /> }
                    </div>
                    <h1 className="headerTwo-font profilePage_username">{currentUser.firstName}</h1>
                ...
                <div className="profilePage_teaCategoryFilter">
                    <form onSubmit={(e) => e.preventDefault()}>
                        <div className="profilePage_reviewOption">
                            <input readOnly onChange={changeCategory} checked={tasted_option_state} type="radio" className="profilePage_teaCategoryInput" name="profilePage_teaCategoryFilter" id="profilePage_teaCategoryInput1" value="коллекция" />
                            <TokenSecond><label htmlFor="profilePage_teaCategoryInput1">Коллекция</label></TokenSecond>
                        </div>
                        <div className="profilePage_reviewOption">
                            <input readOnly onChange={changeCategory} checked={wanted_option_state} type="radio" className="profilePage_teaCategoryInput" name="profilePage_teaCategoryFilter" id="profilePage_teaCategoryInput2" value="Желания" />
                            <TokenSecond><label htmlFor="profilePage_teaCategoryInput2"><label htmlFor="profilePage_teaCategoryInput2" style={{fontSize: "28px", cursor: "pointer", margin: "0px", marginBottom: "1.5px"}}>♡</label>Желания</label></TokenSecond>
                        </div>
                    </form>
                </div>
                {tastedTeaContainer}
